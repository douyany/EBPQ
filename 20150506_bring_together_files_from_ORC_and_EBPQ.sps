...
add files file='H:\prep_orc\orc_sorted_oct31.sav'
 ...
 /keep=UserID CountyID Q1 to Q115.
 
include file='H:\prep_orc\ORC.sps'.
include file='H:\prep_orc\adding_means_of_subscales.sps'.
...
save outfile='h:\prep_ebpq\data_files\20150506_prepped_ORC_file.sav'
 /keep=CountyCode OthersID answeredORC revMotiv revOrgClim Resources StaffAtt.
...
match files
 /file='h:\prep_ebpq\data_files\20150506_prepped_ORC_file.sav'
 /file=*
...
* add EBPAS measures into EBPQ data.
include file='H:\prep_ebpq\do-files\add_ebpas_measures.sps'.

/* the only people who should be in both datasets  */
/* should have a value of zero for the non-match */

temporary.
select if sysmis(EBPAS_Openness)=0.
crosstabs
 /tables=WhyNonMatch by revMotiv.

formats  revMotiv revOrgClim Resources StaffAtt (F9.0). 
formats  EBPAS_Requirements EBPAS_Appeal 
 EBPAS_Openness  EBPAS_Divergence  EBPAS_Total (F9.0). 
...
formats  EBPAS_Requirements EBPAS_Appeal
 EBPAS_Openness  EBPAS_Divergence  EBPAS_Total (F9.0).
CORRELATIONS
  /VARIABLES=revMotiv revOrgClim Resources StaffAtt EBPAS_Appeal EBPAS_Openness EBPAS_Divergence EBPAS_Total
  /PRINT=TWOTAIL NOSIG
  /MISSING=PAIRWISE.
...
temporary.
select if sysmis(EBPAS_Total)=0.
FREQUENCIES VARIABLES=StaffAtt
  /NTILES=4
  /ORDER=ANALYSIS.

 
/* create percentiles */
if (StaffAtt<34.33) staffattrpctile=1. 
if (StaffAtt>=34.33 and StaffAtt<36.63) staffattrpctile=2. 
if (StaffAtt>=36.63 and StaffAtt<39.43) staffattrpctile=3. 
if (StaffAtt>=39.43) staffattrpctile=4. 
if (sysmis(EBPAS_Total)) staffattrpctile=$sysmis.

formats staffattrpctile (f9.0).
variable labels staffattrpctile 'ORC Staff Attributes Quartile'.
value labels staffattrpctile 1 'Lowest' 2 '2nd' 3 '3rd' 4 'Highest'.
...
* Chart Builder.
GGRAPH
  /GRAPHDATASET NAME="graphdataset" VARIABLES=EBPAS_Total StaffAtt MISSING=LISTWISE REPORTMISSING=NO
  /GRAPHSPEC SOURCE=INLINE.
BEGIN GPL
  SOURCE: s=userSource(id("graphdataset"))
  DATA: EBPAS_Total=col(source(s), name("EBPAS_Total"))
  DATA: StaffAtt=col(source(s), name("StaffAtt"))
  GUIDE: axis(dim(1), label("EBPAS: Total"))
  GUIDE: axis(dim(2), label("ORC Domain: Staff Attributes"))
   GUIDE: text.title(label("ORC Staff Attributes vs. EBPAS Total Score"))
  ELEMENT: point(position(EBPAS_Total*StaffAtt))
...
* Chart Builder.
GGRAPH
  /GRAPHDATASET NAME="graphdataset" VARIABLES=staffattrpctile EBPAS_Total OthersID CountyCode MISSING=LISTWISE REPORTMISSING=NO
  /GRAPHSPEC SOURCE=INLINE.
BEGIN GPL
  SOURCE: s=userSource(id("graphdataset"))
  DATA: staffattrpctile=col(source(s), name("staffattrpctile"), unit.category())
  DATA: EBPAS_Total=col(source(s), name("EBPAS_Total"))
  DATA: id=col(source(s), name("CountyCode"), unit.category())
  GUIDE: axis(dim(1), label("ORC Staff Attributes Quartile"))
  GUIDE: axis(dim(2), label("EBPAS: Total"))
   GUIDE: text.title(label("Boxplots: EBPAS Total Score by ORC Staff Attributes Quartile"))
  SCALE: cat(dim(1), include("1", "2", "3", "4"))
  SCALE: linear(dim(2), include(0))
  ELEMENT: schema(position(bin.quantile.letter(staffattrpctile*EBPAS_Total)), label(id))
END GPL.
